...
- **NEW: Google Calendar Integration (December 11, 2025)**: Integrated Replit's built-in Google Calendar connector to automatically push all confirmed bookings to the connected Google Calendar (thebarnmi@gmail.com). Calendar events are created when: 1) A customer completes a Stripe payment (via webhook), 2) An admin creates a booking manually. Events include customer name, email, booking details, space/bundle info, payment status, and facility address. Admin can test calendar connection via GET /api/admin/test-calendar endpoint.
